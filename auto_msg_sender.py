...
class AutoPYQSender:
    ...
    def run(self):
        ...
        for r in fdata_list:
            ...
            self.logger.info(f'请求发送聊天消息: {pyq_text_content}, pyq_pic_path_list={pyq_pic_path_list}')
            # resp = wx_cls.send_new_pyq(pyq_text_content=pyq_text_content, pyq_pic_path_list=pyq_pic_path_list)
            msg = f'[太阳]新品种发布😊：\n{pyq_text_content}'
            resp = wx_cls.send_chat_msg(chat_name='测试', text_msg_content=msg, chat_pic_path_list=pyq_pic_path_list)
            self.logger.info(f'请求发送聊天消息结果: {resp}')
            time.sleep(0.5)
